...
| **3 — universe** | Point-in-time market size | ⚠️ **Collected on Hyperliquid only**: `hl_asset_ctxs` (HL `asset_ctxs` open interest, one row per coin per minute, 1,218 days, 2023-05-20 → 2026-09-19, 275,320,095 rows); listings datable from per-date coin presence and Lighter `created_at` in `markets`. **Lighter has no open-interest history at all** (`datasets.md` item 6 — recorder-only), so Lighter is ranked by a *proxy*: trailing quote volume from `lighter_candles`. See the two-universe decision in [`handoff.md`](handoff.md) §2 |
