...
| F2 | **Whether the primary target is funding or funding-net-of-basis. Measured 2026-09-22 by `scripts/measure_basis.py` (`data/phase2/qa/basis_drift.md`), and the switch branch is the live one.** The original threshold read: under ~6 bp of 24-hour drift sd on traded prices the funding target stands at H=24; near 22 bp the primary target becomes funding **net of basis**. Measured on the repaired candles, sd(Δbasis) at 24h is **19.5–24.8 bp across every frozen-filtered cleaning rule** (37.2 bp with nothing excluded), with a median \|24h drift\| of **7.50 bp** and **17.2–17.7%** of 24-hour holds moving further than the 18.91 bp *selective-entry* widest-spread carry. That is nowhere near 6 and squarely at 22, **so Task 3's funding target is not by itself the right target** and the net-of-basis construction is the one Phase 5 onward should carry. **But it is measured on *mark* prices off two different venue references (Hyperliquid oracle, Lighter index), so part of it is a marking convention rather than realizable P&L — the realizable figure needs traded prices, which are not on disk. F2 therefore STAYS OPEN and Phase 9 sizes nothing until it resolves.** Context: Phase 3 Task 5 measured net carry at **−22 to −32 bp** across every size decile and every age bucket, so nothing in this universe is tradeable at a 24-hour hold on today's evidence regardless of which target is primary. | D6, measured on **traded** prices — the only thing that closes this. Under ~6 bp sd at 24h, keep funding. Near 22 bp, switch. The mark-based measurement above already reads as "switch"; it cannot settle the question because it cannot separate convention from P&L. |
