...
| Total AWS spend recorded | ✅ **$0.01184** across 89 billed S3 operations (33 list, 28 head, 28 get), from `data/phase1/aws_ledger.jsonl`. Cap was $0.80; never approached. **Correction (2026-09-21):** the ledger priced all egress at $0.09/GB, but the two buckets are in different regions — `hyperliquid-archive` is **us-east-1** ($0.09/GB) and `hl-mainnet-node-data` is **ap-northeast-1** ($0.114/GB), each confirmed by the `x-amz-bucket-region` header. Repricing the ledger per bucket (archive 0.0875 GB over 61 calls, node data 0.0392 GB over 28) gives a true list-price spend of **$0.01278**, $0.0009 above the recorded $0.01184. An earlier attempt at this correction put it at $0.0149 by pricing *both* buckets as Tokyo; that was wrong — the ap-northeast-1 endpoint answers `301` for the archive, and a redirect is not evidence of region. `src/fundr/sources/hl_archive.py` now prices per bucket; the evidence notes record what was measured at the time and are left as they are |
