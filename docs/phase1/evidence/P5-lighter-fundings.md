...
- **Timestamp convention**: `timestamp` is Unix seconds, on the hour boundary (e.g. `1737100800` = `2025-01-17 08:00:00 UTC` exactly). **Verified in Task 12 (P9)**: a `/fundings` row timestamped `T` reports the interval that just ended at `T`, i.e. it covers `[T-1h, T)`. Checked live 2026-09-19 ~23:53:58 UTC (market_id 1, `GET /api/v1/fundings`): the latest available row was timestamped `2026-09-19 23:00:00 UTC` — the settlement for `[22:00, 23:00)`, which had already closed — with no row yet for the interval `[23:00, 00:00)` still accruing at request time. So `settle_time` marks the *close*, not the open, of the interval it reports.
