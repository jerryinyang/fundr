...
- The P9 recording's Lighter websocket data ends at **15:19:28Z**, not at the 16:54:35Z run end originally quoted in P9's note (the later timestamps are a `ws_reconnect` and a `poll_error` record, not data). Usable full hours are 11:00–14:00 UTC, which is what caps the sample at 28 market-hours / 16 off-baseline. P9's note has since been corrected, and the cause diagnosed there ("Why the Lighter websocket recording stopped at 15:19:28Z"): the feed was healthy; the recorder only writes websocket snapshots after a successful REST poll, and one REST call hung for 84 minutes.
