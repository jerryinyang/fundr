...
Note: `recent` has only 70/70 coin-hours in both alignments (the alignment-A/B joined denominator; unrelated to the "old 70/70" sign-check count below, which is a different date and a different population) (vs 144 on old/mid) because the archived file for 2026-09-18 is a partial day — P1 found it ends at 09:55 UTC, so settled hours after that have no archived counterpart; `drop_nulls`/`inner join` already drop them, they are not a bug.
...
- **Units, sign, interval** (from https://hyperliquid.gitbook.io/hyperliquid-docs/trading/funding, quoted directly):
  ...
  - Sign — **verified (docs + data agree)**: docs say "If the contract's price is higher than the oracle price, the premium and hence the funding rate will be positive, and the long position will pay the short position" — i.e. positive `fundingRate` should track positive `premium` (both push in the same direction relative to the interest-rate baseline). Checked directly against data, not just docs: on the three `settled_<day>.parquet` files, for every off-baseline coin-hour (`|funding_rate - 0.0000125| > 1e-9`), `sign(funding_rate - 0.0000125) == sign(premium)`. Command:
    ...
    ```
    Result: old 70/70, mid 117/117, recent 58/58 — **245/245 (100%)** off-baseline coin-hours have matching sign between the funding-rate deviation and premium. Sign convention is confirmed both from docs and directly from data. **Note this "old 70/70" is the sign-check count for the `old` date, not the recent-date joined denominator quoted above (also "70/70") — same digits, unrelated populations.** It is also a different, larger population than the 199 (=64+108+27) Alignment-A off-baseline count above: Alignment A's off-baseline filter is `funding_rate != 0.0000125` (exact-equality baseline test), while this sign check's off-baseline filter is the looser `|funding_rate - 0.0000125| > 1e-9` — so 245 (70+117+58) includes some near-baseline coin-hours that Alignment A's exact-equality split classified as baseline. **199 and 245 are two different off-baseline populations and should not be added, subtracted, or conflated.**
